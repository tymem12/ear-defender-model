...
class ValidationDataset(Dataset):
    def __init__(
        ...
        self.samples = pd.DataFrame()  # dataframe with path to the file and other informations (like domain )
...
def apply_trim(waveform, sample_rate):
    ...
    This function, apply_trim, trims silence from the beginning and end of an audio waveform
    waveform: A tensor representing the audio waveform, where each value corresponds to the amplitude of the audio signal at a specific point in time.
    sample_rate: The sample rate of the waveform, which indicates how many samples per second the audio is recorded at.
    """
    (
        waveform_trimmed,
        sample_rate_trimmed,
