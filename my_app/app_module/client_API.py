...
async def connector_create_predictions(analysis_id: str, payload: Dict, token: str) -> Tuple[int, str]:
    ...
    logging.info("Model ready to send predictions to Connector")
    try:
        async with httpx.AsyncClient() as client:
            response = await client.put(connector_url, json=actual_payload, headers=headers)
            response.raise_for_status()
    except httpx.RequestError as e:
        logging.error(f"Failed to connect to the analysis API: {str(e)}")
        return 500, str(e)  # Generic error code for connection issues
    except httpx.HTTPStatusError as e:
        logging.error(f"HTTP error occurred: {str(e)}")
        return e.response.status_code, f"HTTP error occurred: {str(e)}"
